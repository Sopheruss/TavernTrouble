...
namespace SoftwareProjekt2024.Components.StaticObjects
{
    internal class SaladCrate : StaticObject
    {
        ...
        public static void HandleInteraction(PerspectiveManager _perspectiveManager, Vector2 positionWhilePickedUp, Player _ogerCook, InteractionManager interactionManager, InputManager inputManager)
        {
            ...
            {
                interactionManager._interactionTextline = "Press [E] to grab salad";
            ...
            else if (!_ogerCook.inventoryIsEmpty() && _ogerCook.inventory[0] is Salad && !(_ogerCook.inventory[0] as Salad).isPrepared())
            {
                interactionManager._interactionTextline = "Press [E] to put salad back";
                interactionManager._allowedInteraction = true;
                if (inputManager.pressedE)
                {
